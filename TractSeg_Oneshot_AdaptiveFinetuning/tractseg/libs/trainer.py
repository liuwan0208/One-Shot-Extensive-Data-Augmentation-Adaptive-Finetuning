...
def train_model(Config, model, data_loader):
    ...
    for epoch_nr in range(Config.NUM_EPOCHS):
        ...
        for type in types:##["train", "validate"]
            ...
            if type=='train':
                nr_batches = int(int(nr_of_samples / Config.BATCH_SIZE) * Config.EPOCH_MULTIPLIER_train)
                print('nr_batches:',nr_batches)

            if type=='validate':
                nr_batches = int(int(nr_of_samples / Config.BATCH_SIZE) * Config.EPOCH_MULTIPLIER_val)
            ...
            for i in range(nr_batches):
                ...
                if type == "train":
                    nr_of_updates += 1
                    probs, metr_batch = model.train(x, y, weight_factor=weight_factor) ## use the model with config to feedforward and update params
                elif type == "validate":
                ...
                print_loss.append(metr_batch["loss"]) # loss of several batch
        ...
        if Config.LR_SCHEDULE: ## True
            if Config.LR_SCHEDULE_MODE == "min":## True
                model.scheduler.step(metrics["loss_validate"][-1])
                model.pscheduler.step(metrics["loss_validate"][-1])##------for spottune
                model.cscheduler.step(metrics["loss_validate"][-1])##------for spottune

            else:
                model.scheduler.step(metrics["f1_macro_validate"][-1])
            # model.print_current_lr()
            model.print_current_lr_update()

        # Save Weights
        start_time_saving = time.time()
        if Config.SAVE_WEIGHTS: ## True
            # model.save_model(metrics, epoch_nr, mode=Config.BEST_EPOCH_SELECTION)
            model.save_model_update(metrics, epoch_nr, mode=Config.BEST_EPOCH_SELECTION)
...
def predict_img(select_path,direction,Config, model, data_loader, probs=False, scale_to_world_shape=True, only_prediction=False,
    ...
    for batch in tqdm(batch_generator):
        ...
        if Config.DROPOUT_SAMPLING:#False
            ...
            layer_probs = model.predict(x, select_path,direction,idx)  # (bs, x, y, nr_classes)
            # print('*******TEST through this way')
