...
def load_checkpoint(path, **kwargs):
    ...
    # print('param_group name:', checkpoint.keys()) 
    for key, value in list(kwargs.items()):
...
def load_checkpoint_update(path, **kwargs):
    checkpoint = torch.load(path, map_location=lambda storage, loc: storage)
    ##---['unet'] or ['pnet']---name of param group defined when saving checkpoint
    # print('param_group name:', checkpoint.keys()) 
    ##--------------parmas:module.contr_1_1.weight, module.contr_1_1_source.weight, ...----------------
    for key, value in list(kwargs.items()):##----key:unet; value:self.net
        if key in checkpoint:
            if isinstance(value, torch.nn.Module) or isinstance(value, torch.optim.Optimizer):
                ##---new untrained model as dict; structure:[{'parm1'"" tensor},{}..}, ...]
                network_dict = value.state_dict()  
                ##---pretrained model as dict value.state_dict(); structure:[{'parm1'"" tensor},{}..}, ...]
                pretrained_dict = checkpoint[key]  
                ##---param_s in model for testing
                if len(network_dict)==len(pretrained_dict): 
                    # print('----len(network_dict) is equal to len(pretrained_dict)', len(network_dict))
                    value.load_state_dict(pretrained_dict, strict=True)##--self.net.load_state_dict(...)
                ##---param_s not in pretrained model for training    
                else: 
                    # print('-----len(network_dict) is not equal to len(pretrained_dict)', len(network_dict), len(pretrained_dict))
                    for param_t in list(pretrained_dict.keys()):#module.expand_4_1.0.bias
                        param= param_t.split('.')
                        param_name_s = param[1]+'_source'
                        str_c='.'
                        param_s = str_c.join([param[0],param_name_s,param[2],param[3]]) if len(param)==4 else str_c.join([param[0],param_name_s,param[2]])
                        ##---add source param and value into the checkpoint dict
                        pretrained_dict[param_s]=deepcopy(pretrained_dict[param_t]) ##--deepcopy(tensor)
                    # print('--------After COPY, len(network_dict), len(pretrained_dict):', len(network_dict), len(pretrained_dict))
                    value.load_state_dict(pretrained_dict, strict=True)##--self.net.load_state_dict(...)
                    # print(pretrained_dict['module.expand_4_1.0.bias'], pretrained_dict['module.expand_4_1_source.0.bias'] )
            else:##false
                kwargs[key] = checkpoint[key]
    return kwargs


def load_checkpoint_updata_v2(path, **kwargs):
    checkpoint = torch.load(path, map_location=lambda storage, loc: storage)
    ##---['unet'] or ['pnet']---name of param group defined when saving checkpoint
    # print('param_group name:', checkpoint.keys()) 
    ##--------------parmas:module.contr_1_1.weight, module.contr_1_1_source.weight, ...----------------
    for key, value in list(kwargs.items()):##----key:unet; value:self.net->cnet
        # print('key in checkpoint',key in checkpoint, 'isinstance(value, torch.nn.Module) or isinstance(value, torch.optim.Optimizer)',isinstance(value, torch.nn.Module) or isinstance(value, torch.optim.Optimizer))
        if key in checkpoint:
            if isinstance(value, torch.nn.Module) or isinstance(value, torch.optim.Optimizer):
                ##---new untrained model as dict; structure:[{'parm1'"" tensor},{}..}, ...]
                # network_dict = value.state_dict()  
                ##---pretrained model as dict value.state_dict(); structure:[{'parm1'"" tensor},{}..}, ...]
                pretrained_dict = checkpoint[key]
                for param_t in list(pretrained_dict.keys()):#-->self.net
                    param= param_t.split('.')
                    param_name_s = param[1]+'_source'
                    str_c='.'
                    param_s = str_c.join([param[0],param_name_s,param[2],param[3]]) if len(param)==4 else str_c.join([param[0],param_name_s,param[2]])
                ...
                value.load_state_dict(pretrained_dict, strict=False)##--self.net.load_state_dict(...)
                # print(pretrained_dict['module.expand_4_1.0.bias'], pretrained_dict['module.expand_4_1_source.0.bias'] )
            else:##false
                kwargs[key] = checkpoint[key]
    return kwargs
...
def load_checkpoint_selectively(path, **kwargs):
    ...
    for key, value in list(kwargs.items()):
        ...
        if key in checkpoint:
            ...
            remove_layers = ['module.conv_5.weight', 'module.conv_5.bias']
